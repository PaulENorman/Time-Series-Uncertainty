...
def main():
    ...
    for ax, (name, b) in zip(axs, CASES):
        analytic, preds = compute_curves(b, estimators=HYBRID_METHODS, t_generate=300.0, n_signals=8)
        ax.loglog(TIME_POINTS, analytic, "k-", lw=2.0, label="Analytic")
        for k in HYBRID_METHODS:
    ...
    fig.suptitle("Method snapshots across regimes")
    out = FIG_DIR / "figure_11_methods_regimes.png"
    plt.tight_layout()
