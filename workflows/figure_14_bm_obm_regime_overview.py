...
def main():
    ...
    for ax, (name, b, regime) in zip(axs, CASES):
        ...
        ax.loglog(TIME_POINTS, preds["BM"], color="tab:blue", marker="o", markersize=3.4, linewidth=1.6, label="BM")
        ax.loglog(TIME_POINTS, preds["OBM"], color="tab:orange", marker="s", markersize=3.4, linewidth=1.6, linestyle="--", label="OBM")
    ...
    fig.suptitle("Figure 14 (BM+OBM) — Behavior across under/near/resolved/overresolved regimes", y=0.995)

    FIG_DIR.mkdir(parents=True, exist_ok=True)
    out = FIG_DIR / "figure_14_bm_obm_regime_overview.png"
    plt.tight_layout()
